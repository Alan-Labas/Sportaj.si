...
app.use(express.static(path.join(__dirname, '../slike')));

app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, '../spletnaStran2/html/index.html'));
});

app.get('/api/vseAktivnosti', async(req,res)=>{
    try{
        const vseAktivnosti = await knex('sportna_aktivnost').select();
        res.status(200).json(vseAktivnosti)
    }catch(error){
        res.status(500).json({message: "Napaka:", err: error.toString()})
    }
})
app.get('/api/vsiTrenerji', async(req, res)=>{
    try{
        const vsiTrenerji = await knex('trenerji').select();
        res.status(200).json(vsiTrenerji)
    }catch(error){
        res.status(500).json({message: "Napaka:", err: error.toString()})
    }
})
